...
// Khởi tạo biến để lưu trữ tổng số lượng Contributor
$totalContributors = 0;
...
if(isset($_POST['submit'])){
    ...
    if ($_SESSION['role'] == 2) {
        ...
        $sql = "SELECT d.departmentName, COUNT(DISTINCT c.UserID) AS contributor_count         
    ...
    $result = $conn->query($sql);   
    $data = array();
    $data[] = ['Department', 'Contributor Count'];

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $data[] = [$row['departmentName'], (int)$row['contributor_count']];
            $totalContributors += (int)$row['contributor_count'];
        }
    } else {
...
<head>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            ...
            var options = {
                title: '<?php echo ($_SESSION['role'] == 2) ? "Number of Contributors at the Faculty of " . $department_name : "Number of contributors in all Faculties"; ?>',
                pieSliceText: 'label',
                is3D: true,
                chartArea: {
                    left: 50,   
            ...
            if (data.getNumberOfRows() == 1 && data.getValue(0, 0) == 'No Data') {
                document.getElementById('chart-message').innerText = 'No Contributor data available.';
            } else {
                chart.draw(data, options);
                document.getElementById('total-contributors').innerText = 'Total Contributors: <?php echo $totalContributors; ?>';
            }
        }
    </script>
</head>
